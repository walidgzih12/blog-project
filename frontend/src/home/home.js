...
function Home() {
  ...
  const fetchAllBlogs = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/blog/getblogs');
  ...
  const handleCreateNewPost = async(e) => {
    try {
        ...
        const response = await axios.post('http://localhost:3001/api/blog/post', blogData, {headers});
        alert('blog created!');
          ...
          blogs.map((blog) => (
            <div key={blog.id} className='col'>
              <div className='bg-light'>
                ...
                {/* <img className='img' src={blog.image} alt='blog-img' /> */}
              </div>
            </div>
          ))
        ) : (
